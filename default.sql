...
DELETE FROM mysql.user WHERE User = 'dev-database';
CREATE USER 'dev-database'@'localhost' IDENTIFIED BY 'B32sU3PvJkrO';

-- grant only the necessary privileges to our new user
GRANT SELECT, INSERT, UPDATE, DELETE ON `dev`.* TO 'dev-database'@'localhost';
